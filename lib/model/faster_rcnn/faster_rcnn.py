...
class _fasterRCNN(nn.Module):
    ...
    def load_contrastive_backbone(self, state_dict):
        RCNN_base_dict = OrderedDict({
            k.replace('RCNN_base.', ''): v for k, v in state_dict.items() if "RCNN_base" in k
        })
        self.RCNN_base.load_state_dict(RCNN_base_dict)

    def load_contrastive_RPN(self, state_dict):
        RCNN_rpn_dict = OrderedDict({
            k.replace('RCNN_rpn.', ''): v for k, v in state_dict.items() if "RCNN_rpn" in k
        })
        self.RCNN_rpn.load_state_dict(RCNN_rpn_dict)
